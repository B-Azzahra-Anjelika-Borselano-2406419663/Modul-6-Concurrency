...
Pada tahap bonus ini, saya membuat fungsi build sebagai alternatif yang lebih baik dari fungsi new pada ThreadPool. Perbedaan utamanya terletak pada cara menangani input tidak valid. new menggunakan assert! yang langsung menyebabkan panic jika size bernilai 0, sedangkan build mengembalikan Result<ThreadPool, PoolCreationError> sehingga error dapat ditangani secara lebih graceful oleh pemanggil fungsi. Pendekatan ini lebih sesuai dengan idiom Rust yang menganjurkan penggunaan Result untuk operasi yang bisa gagal, daripada membiarkan program crash secara tiba-tiba. Dengan menggunakan build, kode di main.rs pun menjadi lebih informatif karena kita bisa menggunakan unwrap_or_else untuk menampilkan pesan error yang jelas sebelum keluar dari program. Secara keseluruhan, fungsi build mencerminkan praktik pemrograman yang lebih robust dan production-ready dibandingkan new yang bersifat lebih sederhana namun kurang aman.